    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            x_shot, x_query = fs.split_shot_query(
                    ...
                    ep_per_batch=ep_per_batch)
            x_shot_label, x_query_label = fs.split_shot_query_label(
                    _.cuda(), n_train_way, n_train_shot, n_query,
                    ep_per_batch=ep_per_batch)
            label = fs.make_nk_label(n_train_way, n_query,
                    ep_per_batch=ep_per_batch).cuda()

            logits = model(x_shot, x_query,x_shot_label).view(-1, n_train_way)
        ...
        for name, loader, name_l, name_a in [
            ...
            for data, _ in tqdm(loader, desc=name, leave=False):
                x_shot, x_query = fs.split_shot_query(
                        ...
                        ep_per_batch=4)
                x_shot_label, x_query_label = fs.split_shot_query_label(
                        _.cuda(), n_way, n_shot, n_query,
                        ep_per_batch=4)
                label = fs.make_nk_label(n_way, n_query,
                        ep_per_batch=4).cuda()

                with torch.no_grad():
                    logits = model(x_shot, x_query,x_shot_label).view(-1, n_way)
                    loss = F.cross_entropy(logits, label)
                    acc = utils.compute_acc(logits, label)
